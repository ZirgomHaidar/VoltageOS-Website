...
export const fetchDeviceList = async (): Promise<string[]> => {
  try {
    const response = await axios.get<string>(
      "https://raw.githubusercontent.com/VoltageOS/vendor_voltage/refs/heads/16.2/voltage.devices",
    )
...
export const fetchDeviceData = async (device: string): Promise<DeviceInfo> => {
  try {
    const response = await axios.get(
      `https://raw.githubusercontent.com/VoltageOS/android_vendor_voltageota/refs/heads/16.2/${device}.json`,
    )
